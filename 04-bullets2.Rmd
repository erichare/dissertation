...
The `rough_cor` or Roughness Correlation is derived by performing a second smoothing step, and subtracting the result from the original signatures. This creates a new signature which eliminates some of the overall structure, allowing global deformations to have less of an influence on the model output. In a typical comparison between two profiles, such as in Figure \ref{fig:roughcorfine}, the roughness correlation may not be needed given the presence of the ccf in the model. In this figure, we see the alignment of profile 8752 with 136676/ The top panel shows the smoothed signature, while the bottom panel shows the doubly smoothed signatures used to compute the roughness correlation. These profiles match, and both ccf (0.6891) and rough_cor (0.7980) provide values indicative of matching.

```{r roughcorfine, echo=FALSE, fig.cap='Alignment of profile 8752 with 136676. The top panel shows the smoothed signature, while the bottom panel shows the doubly smoothed signatures used to compute the roughness correlation. These profiles match, and both ccf (0.6891) and rough_cor (0.7980) provide values indicative of matching.'}
...
p2 <- ggplot(data = final_doublesmoothed, aes(x = y, y = l50, colour = profile_id)) +
    geom_line() +
    theme_bw()
...
Where the roughness correlation is most useful is in a scenario like Figure \ref{fig:roughcorgood}. This figure shows the alignment of profile 40977 with 47600. The top panel shows the smoothed signature, while the bottom panel shows the doubly smoothed signatures used to compute the roughness correlation. Note that these two profiles do not match, yet the ccf is 0.7724. The roughness correlation (-0.0324) correctly indicates the lack of matching. The roughness correlation acts as a check against false positives which can arise when there are significant deformations in the overall structure, as in the case with both these profiles.

```{r roughcorgood, echo=FALSE, fig.cap='Alignment of profile 40977 with 47600. The top panel shows the smoothed signature, while the bottom panel shows the doubly smoothed signatures used to compute the roughness correlation. Note that these two profiles do not match, yet the ccf is 0.7724. The roughness correlation (-0.0324) correctly indicates the lack of matching.'}
...
study_result_wide_col[,-1] <- apply(study_result_wide_col[,-1], 2, function(x) x / sum(x))
