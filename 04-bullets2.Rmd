...
result <- my_ccf %>% 
    filter(profile1_id == myprof$profile_id, 
           compare_id == 3) %>%
    select(profile1_id, profile2_id, ccf, double_cor, D, overlap, matches, cms, sumpeaks) %>%
    collect() %>%
    slice(5:10) %>%
    as.data.frame
```
...
- **D** is the Euclidean vertical distance between each height value of the aligned signatures. This is defined as $D^2 = \frac{1}{\text{\#}t}\sum_t \left[f(t) - g(t)\right]^2$. This is a measure of the total variation between two functions [@clarkson1933definitions].
...
- **cms** (millimeters) is the number of consecutively matching peaks/valleys (striae) per millimeter of the overlapping portion of the aligned signatures [@biasotti:1959, @thompson:2013].
