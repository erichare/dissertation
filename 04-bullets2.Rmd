...
myresult <- all_combinations %>% map_df(function(x) {
    final_result <- try({
        ...
        result <- set1 %>%
            rbind(set2) %>%
            group_by(feature) %>%
            do(matchtest = ks.test(x = filter(., study == x[1])$value[filter(., study == x[1])$match], y = filter(., study == x[2])$value[filter(., study == x[2])$match])$p.value,
               nonmatchtest = ks.test(x = filter(., study == x[1])$value[!filter(., study == x[1])$match], y = filter(., study == x[2])$value[!filter(., study == x[2])$match])$p.value
            )
    })
