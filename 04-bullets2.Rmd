...
```{r, echo=FALSE, fig.cap='Histograms of matching probability, facetted by the degradation level.', fig.height=10}
...
p1 <- ggplot(data = filter(CCFs_withlands_degrade, match), aes(x = forest)) +
    geom_histogram() +
    facet_grid(degrade_left ~ degrade_right, labeller = label_both, scales = "free_y") +
    theme_bw()
p2 <- ggplot(data = filter(CCFs_withlands_degrade, !match), aes(x = forest)) +
    geom_histogram() +
    facet_grid(degrade_left ~ degrade_right, labeller = label_both, scales = "free_y") +
    theme_bw()
...
result <- CCFs_withlands_degrade %>%
    filter(degrade_left == 0, match) %>%
    select(profile1_id, profile2_id, forest, degrade_left, degrade_right) %>%
    group_by(degrade_right) %>%
    summarise(forest = mean(forest))

ggplot(result, aes(x = degrade_right, y = forest)) +
    geom_point() +
    geom_line() +
    theme_bw() +
    ylim(c(0, 1))
...
ggplot(result, aes(x = degrade_right, y = value, colour = metric)) +
    geom_point() +
    geom_line() +
    theme_bw() +
    ylim(c(0, 1)) +
    xlim(c(0, 1)) +
    xlab("Land Proportion (Left)")
```

```{r, echo=FALSE}
result <- CCFs_withlands_degrade %>%
    filter(degrade_right == 1) %>%
    select(profile1_id, profile2_id, forest, match, degrade_left, degrade_right) %>%
    mutate(pred = (forest > .5),
           degrade_left = 1 - degrade_left) %>%
    group_by(degrade_left, degrade_right) %>%
    summarise(sensitivity = mean(sensitivity(factor(pred), factor(match))),
              specificity = mean(specificity(factor(pred), factor(match)))) %>%
    gather(key = metric, value = value, 3:ncol(.))

ggplot(result, aes(x = degrade_left, y = value, colour = metric)) +
    geom_point() +
    geom_line() +
    theme_bw() +
    ylim(c(0, 1)) +
    xlim(c(0, 1)) +
    xlab("Land Proportion (Right)")
```
