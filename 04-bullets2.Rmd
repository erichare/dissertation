...
```{r}
xtabs(~(forest > 0.5) + match + study.x + study.y, data = CCFs_test)
```
...
includes_study <- setdiff(names(CCFs_train), c("compare_id", "profile1_id", "profile2_id",
...
mytbl <- xtabs(~study_forest + study, data = CCFs_test)
mytbl / colSums(mytbl)
```

Figures \ref{fig:ccfstudy} and \ref{fig:matchstudy} give the distribution of the CCF and the number of matches, respectively, colored by whether the lands are known to be fired from the same gun barrel, across different study to study comparisons.

```{r ccfstudy, message=FALSE, fig.cap="Distribution of the CCF, colored by match, for different study to study comparisons of lands."}
ggplot(CCFs_train, aes(x = ccf, fill = match)) +
    geom_histogram(bins = 50, alpha = I(0.8)) +
    facet_wrap(~study) +
    theme_bw() +
    scale_y_log10()
```

```{r matchstudy, message=FALSE, fig.cap="Distribution of the CCF, colored by match, for different study to study comparisons of lands."}
ggplot(CCFs_train, aes(x = matches, fill = match)) +
    geom_histogram(bins = 50, alpha = I(0.8)) +
    facet_wrap(~study) +
    theme_bw() +
    scale_y_log10()
...
Figure \ref{fig:studystudy} provides panels for the different study to study land comparisons, colored by the predicted study as given by the forest.

```{r studystudy, fig.cap="Study to study comparison of lands, colored by the predicted class."}
ggplot(CCFs_test, aes(x = matches, fill = study_forest)) +
    geom_histogram(bins = 50, alpha = I(0.8)) +
