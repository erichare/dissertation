...
## TODO: put actual and predicted in the tables in ch 3
## switch to scientific notation (or < .0001) for all p-values
## move the KS test table to the appendix for that one paper
## Switch single ks test to a parenthetical in the text
## Facet by sensitivity and specificity, 3 lines for each
## Start that plot at 100%, label it "% land used" - call it "left fixed", "middle fixed", "right fixed"
## Move histograms to the appendix
## Try without signature length?
## Take plot out of first land to land idea
## Feature expression as a function of degradation
## Take a scratched bullet we excluded, (BR9 b24?) and try to match using degraded
## Cary data - same pics as Alan
...
```{r deghist, cache=FALSE, echo=FALSE, fig.cap='Histograms of matching probability, facetted by the degradation level and known match versus known non-match.', fig.height=9, fig.width=8}
...
CCFs_withlands_degrade <- ccf %>%
    ...
    filter(study.x != "Cary", study.y != "Cary", degrade_right - degrade_left >= .5) %>%
    arrange(study.x, study.y) %>%
    mutate(match = as.logical(replace(match, is.na(match), 0)),
...
Figure \ref{fig:senspe} gives the sensitivity and specificity of the random forest for given levels of degradation. It can be seen that both metrics decline as a function of the degradation level, but that the decline for left degradation is less steep.

```{r senspe, echo=FALSE, fig.height=9, fig.width=8, fig.cap='Sensitivity and specificity of the random forest for given levels of degradation. It can be seen that both metrics decline as a function of the degradation level, but that the decline for left degradation is less steep.'}
result <- CCFs_withlands_degrade %>%
    filter(degrade_left == 0) %>%
    dplyr::select(profile1_id, profile2_id, forest, match, degrade_left, degrade_right) %>%
    mutate(pred = (forest > .5)) %>%
    group_by(degrade_left, degrade_right) %>%
...
## Produce plot
p1 <- ggplot(result, aes(x = degrade_right, y = value, colour = metric)) +
    geom_point() +
    geom_line() +
    theme_bw() +
    ylim(c(0, 1)) +
    xlim(c(0, 1)) +
    xlab("Degraded Proportion (Right)")

result <- CCFs_withlands_degrade %>%
    filter(degrade_right == 0) %>%
    dplyr::select(profile1_id, profile2_id, forest, match, degrade_left, degrade_right) %>%
    mutate(pred = (forest > .5)) %>%
    group_by(degrade_left, degrade_right) %>%
    summarise(sensitivity = mean(sensitivity(factor(pred), factor(match))),
...
p2 <- ggplot(result, aes(x = degrade_left, y = value, colour = metric)) +
    geom_point() +
    geom_line() +
    theme_bw() +
    ylim(c(0, 1)) +
    xlim(c(0, 1)) +
    xlab("Degraded Proportion (Left)")

result <- CCFs_withlands_degrade %>%
    filter((degrade_left == 0 & degrade_right == 0) | (degrade_right != 0 & degrade_left != 0)) %>%
    dplyr::select(profile1_id, profile2_id, forest, match, degrade_left, degrade_right) %>%
    mutate(pred = (forest > .5)) %>%
    group_by(degrade_left, degrade_right) %>%
    summarise(sensitivity = mean(sensitivity(factor(pred), factor(match))),
...
p3 <- ggplot(result, aes(x = degrade_left + degrade_right, y = value, colour = metric)) +
    geom_point() +
    geom_line() +
    theme_bw() +
    ylim(c(0, 1)) +
    xlim(c(0, 1)) +
    xlab("Degraded Proportion (Both)")
...
feature_exp <- CCFs_withlands_degrade %>%
    filter(match) %>%
    dplyr::select(degrade_left, degrade_right, ccf, rough_cor, overlap, D, sd_D, matches, mismatches, cms, non_cms, sum_peaks) %>%
    gather(key = feature, value = value, 3:ncol(.)) %>%
    group_by(degrade_left, degrade_right, feature) %>%
    summarise(value = mean(value, na.rm = TRUE)) %>%
    ungroup() %>%
    mutate(fixed = ifelse(degrade_left == 0 & degrade_right == 0, "left", ifelse(degrade_left == 0, "left", ifelse(degrade_right == 0, "right", "middle"))),
