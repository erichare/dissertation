\chapter{ALGORITHMIC APPROACHES TO BULLET MATCHING WITH AN EMPHASIS ON THE DEGRADED LAND CASE}
...
ggplot() +
  geom_line(data = CCFs_tall, aes(x = Feature, y = Value, colour = Type, group = id, alpha = Type, size = Type)) +
  geom_line(data = pcp_data_tall, aes(x = Feature, y = Value, colour = Type, group = Type), size = 2) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_color_manual(values = c("#ff9999", "#e60000", "#b3b3ff", "#000099")) +
  scale_alpha_manual(values = c(.1, .5, .002, .05)) +
