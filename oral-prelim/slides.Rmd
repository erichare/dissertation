...
We need to choose a location (height) of the bullet at which to extract a profile. To do so, we choose a threshold **CCF** to achieve between subsequent profiles:
...
4. If this CCF exceeds a threshold, choose $d_0$ as the signature
5. Otherwise, repeat steps 2 to 4 for $d$, $2d$, $3d$, ... until the threshold is achieved.

$d = 25\mu m, d_0 = 25\mu m$
...
5. Determine the location of the peak of the right groove by finding the first doubly-smoothed value $y_j$ that is the maximum within its smoothing window after reversing the profile. (Similar for valley of right groove).
