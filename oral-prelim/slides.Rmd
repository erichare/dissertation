...
- Thresholds have been established as "standard practice" (e.g., more than 6 consecutively matching striae to declare a match, Nichols 2003)
...
We need to choose a location (height) of the bullet at which to extract a profile. To do so, we choose a threshold **CCF** (Vorburger, 2011) to achieve between subsequent profiles:
...
* *CCF*: Function of the optimum shift distance measuring the correlation between two profiles
* *CMS*: Striated markings that line up exactly with one another without a break or dissimilarity in between them (Similarly, CNMS)
* *Matches*: The number of matches between two signatures (Similarly, Non-Matches)
* *D* = $\sqrt{\frac{1}{\text{\#}t}\sum_t \left[f(t) - g(t)\right]^2}$ where $f(t)$ and $g(t)$ are aligned signatures. The euclidean vertical distance between surface measurements of aligned signatures.
